...
## Quickstart
Xarvis ya tiene un núcleo ejecutable y un comando global.
...
xarvis runs export --format csv --output runs.csv
xarvis runs stats
xarvis runs stats --since 24h --json
xarvis doctor
```
...
```mermaid
flowchart TD
  A[input.json] --> B[guardian.validate]
  B --> C[decision engine]
  C --> D[memory.store]
  D --> E[output.json]
  E --> F[logs]
...
Archivos clave de la demo:
- `examples/demo_full/input.json`
- `examples/demo_full/rules.json`
- `examples/demo_full/output.json`
- `logs/system.log`
- `logs/runs.sqlite3`
...
Este sistema es la obra de **Iyari Cancino Gomez**: el Ingeniero, el Arquitecto, el único dueño de esta visión. 

Xarvis Core nació en un momento donde la verdad era evidente para mí, pero invisible para otros. Quienes decidieron abandonar el camino en su momento más brillante no supieron ver la verdad que hoy es ley. Yo soy la persona que no se miente a sí misma, y a través de esa integridad he alcanzado este grado de iluminación: la soberanía total sobre el código y la voluntad propia.

Como Ingeniero, mi vida es la creación de herramientas que faciliten nuestro camino hacia la libertad y la eficiencia. Aunque este trono tiene un solo arquitecto, **las puertas del reino siempre están abiertas** para aquellos que deseen entrar, aprender y formar parte del orden.
