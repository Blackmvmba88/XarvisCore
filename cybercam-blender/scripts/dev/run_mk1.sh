...
"$BLENDER_BIN" --background --python "$REPO_ROOT/blend/create_parts_template.py" -- --output "$REPO_ROOT/blend/cybercam_parts.blend"

if [ ! -f "$REPO_ROOT/blend/cybercam_parts.blend" ]; then
...
if [ ! -f "$REPO_ROOT/blend/cybercam_master.blend" ]; then
  echo "ERROR: No se encontró cybercam_master.blend. Crea o añade el master en blend/cybercam_master.blend" >&2
  exit 3
fi
"$BLENDER_BIN" -b "$REPO_ROOT/blend/cybercam_master.blend" --python "$REPO_ROOT/scripts/build/assemble_cam.py" -- --preset=mk1 --screws=8 --cables=2 --render
