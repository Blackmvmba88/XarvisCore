...
def _load_rg_webui():
    ...
    sys.modules["rg_webui"] = m
    assert spec.loader is not None
    spec.loader.exec_module(m)
    return m


rg_webui = _load_rg_webui()


@pytest.fixture
def client():
    rg_webui.app.config["TESTING"] = True
...
def test_metrics_endpoint(tmp_path, client):
    ...
    # reload-by-reexec to pick env var
    m = _load_rg_webui()
    app = m.app
...
def test_action_requires_auth_and_approval(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(m.ram_guardian, 'perform_action_and_measure', lambda f, timeout=10: {"pre_free_bytes": 0, "post_free_bytes": 1000, "delta_bytes": 1000, "success": True})
    with m.app.test_client() as c:
        r = c.post('/api/action', headers={'Authorization': 'Bearer s3cr3t'}, json={"action":"quit_app", "target":"Safari"})
        assert r.status_code == 200
        j = r.get_json()
